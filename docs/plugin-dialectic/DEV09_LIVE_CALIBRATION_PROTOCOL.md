...
- **НЕ принимать самопровозглашённые результаты инструментов**: каждый `harness_run`-вызов
  должен быть РЕАЛЬНЫМ tool-вызовом с непустым `task`, а в отчёте — фактический ответ
  bridge (`route_id`, `state`, `reason_codes`), а не пересказ модели. Если `task` пустой
  или `route_id=None` / `ESCALATED` — это считается невалидным прогоном (проверка: отчёт
  обязан содержать подтверждённый `route_id` для каждого заявленного harness_run).
- **НЕ симулировать ролевые диалоги «на бумаге»**: ролевые ответы (Q/A, observer, Advocate)
  либо происходят через реальные Core-циклы, либо явно помечаются как
  `self_modeled: true` и не используются для калибровки observer-порогов.